  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1024,
        system: 'You are an expert art supply advisor. Always respond with ONLY valid JSON, no markdown, no explanation.',
        messages: [{ role: 'user', content: userPrompt }]
      })
    });

    const responseText = await response.text();
    console.log('Kit generation response status:', response.status);
