...
export default function Project() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try { setIdea(JSON.parse(stored)) } catch {}
    }
    if (storedMats) {
      try { setMaterials(JSON.parse(storedMats)) } catch {}
    }
  }, [])
  ...
  if (!idea) {
    return (
      <div style={{ minHeight: '100dvh', background: 'var(--color-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        ...
        <button onClick={() => navigate('/create')} style={{ padding: '12px 24px', background: 'linear-gradient(90deg,#6C3CE1,#FF3D71)', border: 'none', borderRadius: 14, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          Start a project
        </button>
      </div>
    )
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0 8px' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ height: 4, borderRadius: 2, background: i <= 1 ? '#6C3CE1' : 'rgba(255,255,255,0.15)', width: i <= 1 ? 24 : 16 }} />
            ))}
          </div>
          <button
            onClick={() => setSaved(s => !s)}
            style={{ width: 40, height: 40, borderRadius: '50%', background: saved ? '#FF3D71' : 'var(--color-surface)', border: saved ? 'none' : '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <Heart size={18} color="#fff" fill={saved ? '#fff' : 'none'} />
          </button>
        </div>

        {/* Title */}
        <div style={{ textAlign: 'center', padding: '12px 0 20px' }}>
          <h1 style={{ fontSize: 'clamp(22px,5vw,30px)', fontWeight: 800, margin: '0 0 6px', background: 'linear-gradient(90deg,#6C3CE1,#FF3D71)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', lineHeight: 1.2 }}>
            {idea.title}
          </h1>
        ...
        <div style={{ background: 'var(--color-surface)', borderRadius: 16, padding: '12px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button
            ...
            style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '4px 12px', cursor: 'pointer', color: 'var(--color-text-2)', fontSize: 12, fontWeight: 600 }}
          >
        ...
        <div style={{ background: 'var(--color-surface)', borderRadius: 20, padding: 18, marginBottom: 16, border: '1px solid rgba(108,60,225,0.2)' }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#6C3CE1', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          ...
          {current.tip && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, background: 'rgba(239,159,39,0.1)', border: '1px solid rgba(239,159,39,0.25)', borderRadius: 12, padding: '10px 12px', marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 12, color: '#EF9F27', margin: 0, lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', background: 'var(--color-bg)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: activeStep === 0 ? 'var(--color-text-3)' : 'var(--color-text)', fontSize: 13, fontWeight: 600, cursor: activeStep === 0 ? 'not-allowed' : 'pointer', opacity: activeStep === 0 ? 0.4 : 1 }}
            >
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              {steps.map((_, i) => (
                <button key={i} onClick={() => setActiveStep(i)} style={{ width: i === activeStep ? 18 : 7, height: 7, borderRadius: 4, background: i === activeStep ? '#6C3CE1' : i < activeStep ? '#6C3CE180' : 'rgba(255,255,255,0.15)', border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.25s' }} />
              ))}
            </div>
        ...
        {materials.length > 0 && (
          <div style={{ background: 'var(--color-surface)', borderRadius: 20, padding: 18, marginBottom: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {materials.map((m, i) => (
                <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: 'rgba(108,60,225,0.1)', border: '1px solid rgba(108,60,225,0.2)', borderRadius: 20, fontSize: 12, color: 'var(--color-text)' }}>
                  <Paintbrush2 size={10} color="#9b7ff0" /> {m.name || m}
                </span>
              ))}
            </div>
          </div>
